...
public class FriendService implements IFriendService {
	...
	Friends friend = friendDao.findById(id);
	return friend;
    }
	...
	Iterator<Friends> friendsIt1 = friendDao.findByProperty("user", owner)
		.iterator();

	Iterator<Friends> friendsIt2 = friendDao.findByProperty("userFriend",
		owner).iterator();
	...
	if (pageNow * pageSize - pageSize > friends.size()) {
	    ...
	    return friends.subList(pageNow * pageSize - pageSize, friends
		    .size());
	} else {
	    return friends.subList(pageNow * pageSize - pageSize, pageNow
		    * pageSize);
	}
